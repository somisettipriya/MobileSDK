...
/**
 ...
 RESTRICTION_NONE    - No restriction applied.
 RESTRICTION_ANSI_X9 - The image is stored in ANSI X9 TIFF format.
 */
...
/**
 ...
 An instance of this class contains an image representation stored in memory, in a file or both. The class includes everything related to this image, including captured image data and metadata associated with an image after it has been processed. It includes reference information for using an Image object with image processing or image capture completes.\n\n
...
/**
 When MICR reading is requested during image processing, the detected MICR string is stored in this property. When no MICR string is detected or present, this property is set to nil.\n
 \n
 Default: nil
 */
@property (nonatomic, strong) NSString *imageMicrData; // archived


// ==========================================================
//! Global latitude of the location where the original photo was taken.
/**
 This float value contains the latitude where the photo was taken. The library initializes this global position data from the original input image taken by the device's camera. The latitude and longitude values return the global position such as these follows:\n
 Latitude: 33.670502\n
...
/**
 This float value contains the longitude where the photo was taken. The library initializes this global position data from the original input image taken by the device's camera. The latitude and longitude values return the global position such as these follows:\n
 Latitude: 33.670502\n
 Longitude: -117.762001\n
 For Kofax, Irvine.\n
 \n
...
// ==========================================================
//! Disabling exif and gps tags for output image file or buffer metadata
/**
 The imageWriteToFile and imageWriteToFileBuffer methods ("the imageWrite methods")
    store three EXIF (Exchangeable Image File ) tags for date and time into the output image
    file or buffer metadata (JPEG and TIFF only):
    
    - DateTime (code 0x132)
    - DateTimeDigitized (code 0x9004)
    - DateTimeOriginal (code 0x9003)

 If user sets this property to true, the above tags will not be written to the file or buffer metadata along with Exif and GPS tags.
 
 */

@property (assign) BOOL disableExifandGPSTags;
...
/**
 An instance of this class contains the simple image processing options that the image processor used to produce this image. It is set by the image processing engine in the output image it produces. The profile defines the standard crop, deskew, desired rotation and other options. Deskew option applies to image with skew angle less or equal to 15 degrees.
...
/**
 ...
 @deprecated Deprecated in 3.4.
 */

@property (nonatomic, retain) NSMutableArray* classificationResults;
...
/**
 The target frame is a rectangular area confined within the bounds of the image, and represents the ideal area that should be covered by a document.
 This property will be initialized when image is captured using ImageCaptureControl along with one of the Capture Experience.
 \n
 Default: CGRectZero.\n
 */
...
/**
 ...
 The imageWriteToFile and imageWriteToFileBuffer methods ("the imageWrite methods")
	...
	If the imageReadFromFile method has been used with this Image object, the
	imageWrite methods write any JPEG or TIFF date and time stamps from the read image file
	"as is" into the output file or buffer metadata.
	
	Otherwise, the imageWrite methods write internally generated EXIF metadata into the new
	JPEG or TIFF file or buffer as follows (all times adjusted to GMT):
	
	The EXIF tag DateTime is generated based on the date and time the
	image object was created.
	
	EXIF tags DateTimeOriginal and DateTimeDigitized are normally generated the same way as the
	EXIF tag DateTime, but there is a difference when the given image was created as a result of
	image processing. In such cases, the imageWrite methods generate the DateTimeOriginal value
	based on the date/time of the image that was input to image processing. The input image
	date/time is normally the date/time that image object was created, but could also be derived
	from JPEG or TIFF image file metadata if the imageReadFromFile method was used
	with that image object.
	
	The date/time strings are formatted as yyyy:MM:dd HH:mm:ss, as required by the EXIF
	standard. This format does not accommodate time zones, so the the imageWrite methods use
	the UTC time that the Image object was created, with the time shifted to compensate for
	the time zone offset.
	
	However, when the imageReadFromFile method has been used with the Image object, the
	JPEG or TIFF date/time stamp from the original image file metadata is carried "as is" into the
...
// ==========================================================
//! Write the image contained in the UIImage to the file path specified in the object with file restriction
/**
 Use this method to write the image contained in the UIImage object to the file path specified in the object, in the storage format specified in the mimetype. This method writes the bitmap in the imageBitmap of the object to a file using the specified mimetype to store the image with restriction such as a ANSI X9 TIFF.  The file path extension should match the mimetype. The method returns errors for invalid sequences, mismatched mime types, missing file path, etc.\n\n
 The following properties should be set before calling the write method:\n\n
 - The imageFileOutputColor, the output color mode must be KED_BITDEPTH_BITONAL\n
 - Use specifyFilePath, to set the fully qualified path, file name and file extension to which you want to write the image.\n
 - The imageMimeType, which should be linked to the file type extension. This is set for you by the specifyFilePath method. This needs to be set as MIMETYPE_TIF\n
 - The imageDPI, to specify the output image DPI if it has not already been set by a specifyImageBitmap or imageReadFromFile method calls. The allowed values are 200 and 240.\n
 - The jpegQuality, to specify the compression that you want to use, if you want to use a differnt setting than the default.\n
 - The imageBitmap, to set the image you want to write to the file.\n
 - The following kfxKEDImage object properties are set after this method completes without error:\n
 - The imageRepresentation to indicate the object is represented by both the file name and the bitmap image.\n\n
 ...
 - KMC_ED_IMAGE_WRITTEN_IS_NOT_BITONAL, when you set imageFileOutputColor to bitonal, but while bit packing the image it was found to not be true binary.\n
 - KMC_ED_INVALID_ISO_DATE_TIME_FORMAT, if you previously set an invalid date time string.
 - KMC_ED_RESTRICTION_MIMETYPE_MISMATCH, The mimetype setting of the Image object is not compatible with the specified output file restriction.
 - KMC_ED_RESTRICTION_INVALID_DPI, The DPI of the Image object is not compatible with the specified output file restriction.
 - KMC_ED_RESTRICTION_INVALID_COLOR, The OutputColor of the Image object is not compatible with the specified output file restriction.
 ...
 @see specifyImageDateAndTime to change the date associated with the image as needed.
 */
- (int) imageWriteToFile: (FILE_RESTRICTION) restriction;
...
/**
 ...
 @return KMC_SUCCESS if the image was written to the file buffer successfully, or error:\n
  ...
  - KMC_ED_INVALID_ISO_DATE_TIME_FORMAT if you previously set an invalid date time string.\n
  - KMC_ED_OBJECT_REP_NO_BITMAP, when the Image object does not contain an image.\n
  - KMC_ED_MIMETYPE, when the mimetype setting is illegal.\n
...
// =========================================================================
//! Write the UIImage object in this kfxKEDImage object to file buffer in ANSI X9 tiff format
/**
 Use this method to write the image contained in the UIImage object to a buffer included in this object. The buffer resembles the content of an image file, and the buffer is written in the storage format specified by the mimetype property. This method writes the bitmap in the imageBitmap of the object to the buffer using the specified mimetype to store the image as a TIFF, JPG or PNG file buffer representation.\n\n
 The method returns errors for invalid sequences, illegal mime types, invalid DPI, etc.\n\n
 The following properties should be set before calling the imageWriteToFileBuffer method:\n
 - The imageFileOutputColor, the output color mode must be KED_BITDEPTH_BITONAL.\n
 - The imageMimeType, which controls how the buffer is formatted, this must be set as MIMETYPE_TIF.\n
 - The imageDPI, to specify the output image DPI if it has not already been set by a specifyImageBitmap or imageReadFromFile method calls. The allowed values are 200 and 240.\n
 - The imageBitmap, to set the image you want to write to the buffer.\n
 ...
 */
- (int) imageWriteToFileBuffer : (FILE_RESTRICTION) restriction;


// ==========================================================
//! Get the current image size
/**
 Use this method to get the size of the image associated with this object.\n
    Image Representation:
        IMAGE_REP_BITMAP - Calculated size for image width and image height
        IMAGE_REP_BOTH   - Calculated size for image width and image height
        IMAGE_REP_FILE   - Size of the file or buffer containing the image
 
 @returns size of image data in bytes, zero if no image is found for this object..
 */
- (long) getImageSize;
...
/**
 This property is typically set by the image capture control
 This property will be filled in by the ImageProcessor Engine to the imageLatitude of the input Image when returning a processed Image object.\n
 
 If the Image on disk has latitude information in its meta data and user tries to read image then this API will return the latitude value, otherwise it will return zero.
...
/**
 ...
 If the Image on disk has longitude information in its meta data and user tries to read image then this API will return the longitude value, otherwise it will return zero.
...
// ==========================================================
//! Format a simple date/time string
/**
 Use this method to format a date and time in a simple, short format, such as mm/dd/yy, hh:mm.
 */
+(NSString*)simpleDateTimeString:(NSDate*)date;

// ==========================================================
//! Initialize a date object from a date string
/**
 Use this method to initialize a date object given a date string in the format MMM DD, YYYY. The time portion of the NSDate object will not be set.
 */
...
/**
 Use this method to generate a string appropriate for setting into the imageCreateDateTime property. When an instance of this class is created, the library initializes the imageCreateDateTime for you. If you need to change it to a different date and time, you can use this method to create a new string.
